// GET /setup — landing page after a user installs the GitHub App.
//
// GitHub redirects here with `?installation_id=<n>&setup_action=install` after
// the user picks repos. We render a single static HTML form: pick agent
// profile, paste API key, opt into auto-PR, hit Install. All form action
// happens via POST /install.

import { PROFILES } from "../profiles.js";

export function renderSetupPage(args: { installationId: string }): Response {
  const html = renderHtml(args.installationId);
...
function renderHtml(installationId: string): string {
...
// External script (kept out of inline-script CSP bucket). Handles:
//  - reactive API-key label + help link based on selected profile
//  - auto-opening the "custom" disclosure when the form is reset to it
